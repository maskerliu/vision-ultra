...
```mermaid
flowchart TB
    subgraph External["外部资源 / 输入源"]
        Media[("媒体输入<br/>图片 / 视频 / 摄像头 / 屏幕")]
        Models[("模型文件<br/>ONNX / TFLite / Task / JSON")]
        FS[("本地文件系统<br/>用户数据 / 配置 / 输出")]
    end

    subgraph Main["Electron 主进程 Node.js"]
        MA[MainApp<br/>应用生命周期 / 窗口 / 托盘 / 主题]
        MS[MainServer<br/>Express / HTTP2 / HTTPS / 静态资源]
        IPC[IPCServices<br/>系统对话框 / 更新 / 重启]
        IC[IocContainer<br/>Inversify 依赖注入]
        Router[Router Layer<br/>/common /mapi /facerec]
        Service[Service Layer<br/>Common / Mapi / Push / FaceRec]
        CVB[cv.backend<br/>OpenCV Node 原生后端]
        Repo[Model Repo<br/>PouchDB / LevelDB]
    end

    subgraph Bridge["contextBridge preload.cjs"]
        Preload[window.mainApi / window.cvApi]
    end

    subgraph Renderer["Electron 渲染进程 Chromium"]
        Vue[Vue 3 SPA<br/>Vant / TDesign / Vue Router]
        Pinia[Pinia Stores<br/>Common / Vision / Audio / AI]
        API[main.api.ts / CVApi.ts<br/>IPC / HTTP 客户端]
        PM[ProcessorManager<br/>WorkerManager / BackendManager]
        Workers[Web Workers<br/>CV / Detection / Face / OCR]
    end
    ...
    MA -->|窗口创建| Preload
    IPC -->|ipcMain.handle| Preload
    CVB -->|contextBridge.exposeInMainWorld| Preload

    Preload -->|window.mainApi| API
    Preload -->|window.cvApi| PM
    API --> Vue
    Vue --> Pinia
    Vue --> PM
    PM --> Workers
    PM -->|BackendManager| API
    API -->|HTTP/SockJS| MS
