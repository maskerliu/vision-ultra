...
onMounted(() => {
  if (!__IS_WEB__) {
    window.mainApis.onDownloadUpdate((...args: any) => {
...
function onThemeChanged() {
  ...
  if (!__IS_WEB__) window.mainApis.setAppTheme(theme.value)
}
...
async function onVersionCheck() {
  ...
  hasNewVersion.value = false
  try {
    newVersion = await versionCheck(commonStore.bizConfig.platform, commonStore.bizConfig.arch, commonStore.bizConfig.updateServer)
    ...
    hasNewVersion.value = newVersion.version != commonStore.bizConfig.appVersion
...
function downloadNewVersion() {
  ...
  if (!__IS_WEB__) window.mainApis.downloadUpdate(newVersion)
}

function restart() {
  showDownload.value = false
  if (!__IS_WEB__) window.mainApis.relaunch()
}
