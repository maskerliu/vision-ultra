...
export class FaceRecRouter extends BaseRouter {
  ...
  override initApiInfos(): void {
    this.addApiInfo({
      ...
      path: `${Api.F_List}/:keyword`,
      func: 'list', target: 'faceRecService',
      params: [{ key: 'keyword', type: ParamType.Path }]
    })
    this.addApiInfo({
      method: BizNetwork.Method_Post, path: `${Api.F_Registe}`,
    ...
    this.addApiInfo({
      method: BizNetwork.Method_Post, path: `${Api.F_Delete}`,
    ...
    this.addApiInfo({
      method: BizNetwork.Method_Post, path: `${Api.F_Recognize}`,
