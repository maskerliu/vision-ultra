...
| 实例分割 | ✅ 已实现 | 掩膜渲染 |
| 目标跟踪 | ❌ 未实现 | `ObjectTracker.ts` 仅为 YOLO 检测器封装，无跨帧 ID 关联 |
...
> 详细规格参见独立文档 `FEATURE-TRACKING-SPEC.md`

**Why（解决什么问题）**

当前 `ObjectTracker.ts` 仅封装了 YOLO 检测推理，**每帧独立检测，无跨帧 ID 关联**。用户无法知道上一帧的目标是否是这一帧的同一个目标，无法分析移动路径。

**用户故事（P0/P1）**

| # | 用户故事 | 优先级 |
|---|---------|--------|
| US-3-8 | 作为安防分析师，我希望视频播放时系统自动给每个检测目标分配固定 ID 并绘制运动轨迹线，以便直观看到每个目标的完整移动路径 | P0 |
| US-3-9 | 作为分析师，我希望点击某个目标的轨迹线可以高亮该目标并查看其出现时间段，以便做针对性分析 | P0 |
| US-3-10 | 作为研究者，我希望切换 SORT / ByteTrack 算法并调节 IOU 阈值，以便对比不同跟踪算法效果 | P1 |
| US-3-11 | 作为分析师，我希望导出跟踪结果为 MOT Challenge / JSON 格式，以便用第三方工具进一步分析 | P1 |

**功能范围（Goals）**

- 跨帧目标关联（Tracking-by-Detection）：SORT（卡尔曼滤波 + IOU 匹配）、ByteTrack
- 轨迹可视化：每个 Track ID 独立颜色、渐变透明轨迹线、当前帧中心点
- 跟踪控制：开启/暂停/清除轨迹、算法选择、参数调节（IOU 阈值、最大丢失帧数、轨迹线长度）
- 目标详情交互：点击高亮、信息面板（Track ID / 类别 / 首次出现时间 / 出现帧数）
- 数据导出：MOT Challenge 标准格式 + JSON

**Non-goals（本期不做）**

- 不做 DeepSORT（Re-ID 外观特征），留待 Later
- 不做多摄像头跨视角跟踪
- 不做行为识别/动作分类
- 不做自动轨迹统计分析报表
...
| 新增/修改文件 | 内容 |
|--------------|------|
| `common/model/tracker/SortTracker.ts` | **新增**：SORT 算法（卡尔曼滤波 + 匈牙利匹配） |
| `common/model/tracker/ByteTracker.ts` | **新增**：ByteTrack 算法 |
| `common/model/tracker/Track.ts` | **新增**：Track 数据模型（ID / 历史框 / 轨迹点 / 状态） |
| `common/model/ObjectTracker.ts` | 重构：检测与跟踪分离，`detect()` 输出接入跟踪器 `update()` |
| `common/ipc/ObjTrack.worker.ts` | 扩展消息协议，新增 tracking CMD |
| `pages/vision/DetectRec.vue` | 新增跟踪控制按钮、轨迹设置面板、目标详情弹窗 |
| `store/Vision.ts` | 新增 `enableTracking`、`trackAlgorithm` 等状态 |

**验收标准**

| # | 验收项 | 标准 |
|---|--------|------|
| AC-1 | SORT 跟踪准确性 | MOT17 测试视频上 MOTA ≥ 60%（SORT 论文基线） |
| AC-2 | 实时性能 | 跟踪计算耗时 < 5ms/帧（不阻塞检测推理） |
| AC-3 | 遮挡恢复 | 目标遮挡 ≤ maxAge 帧后重新出现，ID 保持一致 |
| AC-4 | 数据导出 | 导出文件可被 TrackEval 工具正确读取 |

**排期估算**：约 11 人日（POC 3日 + 功能完整 5日 + 稳定性 3日）
...
| 未完成项 | 模块 | 风险等级 | 说明 |
|---------|------|---------|------|
| 视频目标轨迹跟踪 | 模块3 | 🟡 中 | `ObjectTracker.ts` 仅为检测器，无跨帧 ID 关联，功能完全未实现 |
...
| 5 | 视频目标轨迹跟踪（SORT/ByteTrack） | 50% | 2 | 80% | 0.5 | **160** | 检测分割 |
| 6 | Audio Store 完善 + 音频可视化 | 20% | 1 | 50% | 1.5 | **6.7** | 音频处理 |
...
| 需求 | 目标 | 验收标准 |
|------|------|---------|
| **视频目标轨迹跟踪** | 检测 + 跨帧 ID 关联 + 轨迹可视化 | SORT MOTA ≥ 60%；跟踪耗时 < 5ms/帧；支持 SORT/ByteTrack 切换 |
...
| 检测抖动导致轨迹抖动 | 中：跟踪准确性下降 | 对检测框做指数移动平均平滑后再送入跟踪器 |
